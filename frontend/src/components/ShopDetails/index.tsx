...
/** Group variant attributes by name with unique values */
function getAttributeOptions(variants: ApiVariant[]): Record<string, string[]> {
  const map: Record<string, Set<string>> = {};
  for (const v of variants) {
    for (const a of v.product_variant_attributes || []) {
...
function findVariantByAttributes(
  ...
  return (
    variants.find((v) => {
      const attrs = v.product_variant_attributes || [];
      return keys.every(
        (k) => attrs.some((a) => a.name === k && a.value === selected[k])
      );
    }) ?? null
  );
}

const ShopDetails = ({ initialProduct, variants = [] }: ShopDetailsProps) => {
  ...
  const attributeOptions = React.useMemo(
    () => (variants.length ? getAttributeOptions(variants) : {}),
    [variants]
  );
  const initialAttributes = React.useMemo(() => {
    const first = variants[0];
    if (!first?.product_variant_attributes?.length) return {};
    const r: Record<string, string> = {};
    for (const a of first.product_variant_attributes) {
      r[a.name] = a.value;
    }
    return r;
  }, [variants]);
  ...
  const gallery = React.useMemo(() => {
    ...
    const add = (url: string, variantId: number | null) => {
      if (!url || seen.has(url)) return;
      seen.add(url);
      items.push({ url, variantId });
    };

    for (const url of product?.imgs?.previews ?? []) {
      add(url, null);
    }

    for (const v of variants) {
      const files = [...(v.files ?? [])].sort((a, b) => a.position - b.position);
      for (const f of files) {
        add(productFileUrl(f.file_id), v.id);
      }
    }
                ...
                <div className="lg:max-w-[570px] w-full">
                  <div className="lg:min-h-[512px] rounded-lg shadow-1 bg-gray-2 p-4 sm:p-7.5 relative flex items-center justify-center">
                    <div>
                      ...
                      {galleryUrls[previewImg] ? (
                        <img
                          src={galleryUrls[previewImg]}
                          alt="products-details"
                          width={400}
                          height={400}
                        />
                      ) : (
                        <div className="flex items-center justify-center w-full min-h-[400px] bg-gray-2 text-dark-4 text-custom-sm">
                          No image
                        </div>
                      )}
                    </div>
                  ...
                  {/* Thumbnails - only when available */}
                ...
                <div className="max-w-[539px] w-full">
                  ...
                  <form onSubmit={(e) => e.preventDefault()}>
                    <div className="flex flex-col gap-4.5 border-y border-gray-3 mt-7.5 mb-9 py-9">
                      {/* Variant attributes from API */}
                      {variants.length > 0 &&
                        Object.entries(attributeOptions).map(([attrName, values]) => (
                          <div key={attrName} className="flex items-center gap-4">
                              ...
                              {values.map((value) => (
                                ...
                                >
                                  <input
                                    ...
                                    id={`${attrName}-${value}`}
                                    className="sr-only"
                                    checked={selectedAttributes[attrName] === value}
                                    onChange={() =>
                                  ...
                                  />
                                  <span
                                    className={`inline-flex py-1.5 px-3 rounded border text-custom-sm ${
                                      selectedAttributes[attrName] === value
                                        ? "border-blue bg-blue text-white"
                                        : "border-gray-4 hover:border-blue"
                                    }`}
                                  >
                                    {value}
                                  </span>
                      ...
                      {/* Placeholder options when no variants */}
                      {variants.length === 0 && (
                        <>
                      {/* <!-- details item --> */}
                    ...
                    <div className="flex flex-wrap items-center gap-4.5">
                      ...
                      {variants.length > 0 ? (
                        <>
                          ...
                          {addToCartError && (
                            <p className="text-red text-custom-sm">{addToCartError}</p>
                          )}
                          {variants.length > 0 && !canAddToCart && !addToCartLoading && (
                            <p className="text-dark-4 text-custom-sm">Out of stock</p>
                          )}
                        </>
                      ) : null}
