...
const PreviewSliderModal = () => {
  const { closePreviewModal, isModalPreviewOpen, previewIndex } =
    usePreviewSlider();

  const data = useAppSelector((state) => state.productDetailsReducer.value);
  const images = data.imgs?.previews?.filter(Boolean) ?? [];
  ...
  return (
    ...
    >
      ...
      {images.length > 0 ? (
        <Swiper
          key={`${isModalPreviewOpen}-${previewIndex}-${images.length}`}
          ref={sliderRef}
          slidesPerView={1}
          spaceBetween={20}
          initialSlide={Math.min(previewIndex, images.length - 1)}
        >
          {images.map((src, i) => (
