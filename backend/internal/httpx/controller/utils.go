...
func GetMultiPartFile2(r *http.Request, key string, maxSize int64, requiredContentType string) (model.FileStream, error) {
	file, header, err := r.FormFile(key)
	if err != nil {
		err = fmt.Errorf("key %q missing: %w", key, ErrMissingFile)
		return nil, err
	}
	...
	// Reset back to beginning after MIME detection.
